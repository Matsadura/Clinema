...
export default function SignUp({ slideRun }) {
	...
	const [ConfPassword, setConfPassword] = useState("polymorphins");
	...
	function hodnleSubmit(e) {
		...
		fetch('http://127.0.0.1:5000/api/register', request_header)
			.then((res) => res.json())
	...
	return <div className="mx-auto w-full max-w-sm lg:w-96">
		<div className="flex flex-col items-center">
			<img
				...
				alt="Your Company"
			/>
			<h2 className="mt-6 text-3xl font-bold tracking-tight text-white text-center testing">Create you account</h2>
		</div>

		<div className="mt-8">
			<div className="mt-6">
				<form action="#" onSubmit={hodnleSubmit} method="POST" className="space-y-6">
					{/* Name inputs */}
					...
					<div className="space-y-1">
						...
						<div className="mt-1">
							<input
								...
								value={password}
								required
								className={`block w-full text-white bg-secondary-dark ${errPassowrd ? 'border-2 border-red-400 focus:shadow-red-400' : 'border border-primary focus:shadow-primary'} rounded-md px-3 py-2 placeholder-gray-500  focus:shadow-sm  focus:outline-none sm:text-sm`}
							/>
						</div>

						{errPassowrd ? <span className="pl-3 pt-1 block text-sm text-red-100">The password confirmation does not match.</span> : null}
					</div>
					...
					<div className="p-0 m-0">
						<span className="text-sm bg-secondary-dark rounded-md px-2 pt-2 pb-2 flex items-center text-red-400">{error || <br />}</span>
					</div>
					<div>
						<button
							type="submit"
					...
					<div className="text-white select-none">
						You already have an account? <a className="text-primary hover:text-green-300 cursor-pointer" onClick={slideRun}>Log in</a>
					</div>
				</form>
			</div>
		</div>
